...
> ⚠️ **이 표는 2026-07-24 기준이라 아래쪽 ❌ 항목 상당수가 이미 낡았다.**
> 특히 **매칭·확정(§4)은 구현돼 있다** — Action 3종(생성·확정·취소), 조건 대조 추천 엔진,
> 형제 그룹 배정, 관리자 앱 API, 콘솔 화면(`/admin/screen/matching`)까지 동작한다.
> 입국·이송, 근무상태 종합 점검표(43항목·서명·제출 PDF·관계기관 이메일 제출), 근로자 개인 서류,
> 긴급 SOS, 생활 체크리스트도 마찬가지다.
> **지금 무엇이 있는지는 [docs/NDN_전체_화면_버튼_매뉴얼.md](docs/NDN_전체_화면_버튼_매뉴얼.md) 를 기준으로 볼 것.**
> 이 파일은 그때의 판단 기록으로 남겨 둔다.

> **2026-07-24 업데이트 — MVP 우선순위 6종 백엔드 구현 완료** (테스트 58 passed):
> ① 월별 인터뷰(6항목·이탈 리스크 스코어) ② 민원 티켓(+근로자 API) ③ 후보자·면접평가(+대기열·자동충원)
> ④ 정착 처리보드(칸반·SLA·동의 게이팅) ⑤ 파기 잡(`workers:purge-expired`+스케줄) ⑥ 지자체 월간 보고 PDF(dompdf).
> 관리 콘솔에 후보자·정착보드·월별점검·민원 화면 추가. 아래 표의 해당 항목은 ✅ 로 갱신됨.
