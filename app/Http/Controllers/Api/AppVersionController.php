...
/**
 * 모바일 앱 버전 확인 (사이드로딩 배포).
 *
 * 플레이스토어를 거치지 않아 스토어의 자동 업데이트가 없다. 앱이 시작·복귀할 때
 ...
 * 띄울 수 있어야 하기 때문이다.
 *
 * 공개되는 값은 버전 번호와 이미 공개된 APK 주소뿐이라 개인정보가 없다(§7).
 * 근로자·관리자 앱이 같은 바이너리라 두 앱 모두 이 응답을 쓴다.
 */
class AppVersionController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        // version_code 를 안 보낸 구버전·외부 호출에는 안내만 하고 강제하지 않는다.
        ...
        return response()->json([
            ...
            // 강제 기준을 앱에 두면 구버전이 스스로를 강제할 수 없다.
